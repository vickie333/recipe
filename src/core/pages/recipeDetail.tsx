...
import { Clock, DollarSign, ChevronLeft, Edit, Trash2, ExternalLink, UtensilsCrossed, TagIcon } from "lucide-react"
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2 bg-primary/5 px-4 py-2 rounded-lg border border-primary/10">
              ...
              <div>
                ...
                <p className="font-semibold">{formatTime(recipe.time_minutes)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 bg-primary/5 px-4 py-2 rounded-lg border border-primary/10">
              ...
              <div className="grid grid-cols-2 gap-2">
                {recipe.ingredients.length > 0 ? (
                  recipe.ingredients.map((ing) => (
                    <div key={ing.id} className="flex items-center gap-2 p-2 rounded-md bg-muted/50 text-sm border">
                      <div className="w-1.5 h-1.5 rounded-full bg-primary" />
                      {ing.name}
                    </div>
                  ))
                ) : (
                  <p className="text-sm text-muted-foreground italic col-span-2">No ingredients listed</p>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="pt-8 border-t">
        ...
        <div className="prose prose-orange max-w-none">
          {recipe.description ? (
            <div className="whitespace-pre-wrap text-muted-foreground leading-relaxed">{recipe.description}</div>
          ) : (
            <p className="text-muted-foreground italic">No description provided for this recipe.</p>
          )}
        </div>
      </div>
    </div>
